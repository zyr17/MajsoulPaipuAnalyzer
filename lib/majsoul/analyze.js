...
function reporterror(data){
    ...
    function sendmain(data){
        let dataurl = encodeURIComponent(JSON.stringify(data)), STEP = 8000;
        for (let i = 0; i < dataurl.length; i += STEP){
            let url = `http://t.zyr17.cn/MajsoulPaipuAnalyzer/ErrorCollect?i=${i}&data=${dataurl.slice(i, i + STEP)}`;
