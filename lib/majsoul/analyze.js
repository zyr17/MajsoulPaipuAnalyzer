...
const modeid2room = [-1, 1, 1, 1, 2, 2, 2, 3, 3, 3, 4, 4, 4, 101, 101, 5, 5];

var ProtobufRoot, ProtobufWrapper, ProtobufRequest;
function ProtobufInit(liqijson){
...
function analyzeGameRecord(record_list){
    for (let i = 0; i < record_list.length; i ++ ){
        ...
        if (paipugamedata[record.uuid] != undefined)
        ...
        if (record.config.category == 1){
            //友人场
            gamedata.roomdata.room = 0;
        }
        else if (record.config.category == 2){
            //段位场
            gamedata.roomdata.room = modeid2room[record.config.meta.mode_id];
            if (gamedata.roomdata.room == 101){
                //TODO: 古役场 目前不记录牌谱数据，以后可以考虑加入以及如何标记
                gamedata.roomdata.room = 0;
                continue;
            }
        }
            ...
            //比赛场
            gamedata.roomdata.room = 0;
            gamedata.roomdata.contest_id = record.config.meta.contest_id;
        }
        else{
            reporterror({ message: '未知牌谱类别编号' + record.config.category, data: data});
            return undefined;
        }
        ...
        if (record.config.mode.mode >= 20 || roundtype > 4){
            reporterror({message: '未知房间模式编号' + record.config.mode.mode, data: data});
            return undefined;
        }
        ...
        if (gamedata.roomdata.player != gamedata.playerdata.length || 
            gamedata.roomdata.player != record.result.players.length){
                reporterror({message: '玩家数目相关条目不统一', data: data});
                return undefined;
        }
        ...
        paipugamedata[gamedata.uuid] = gamedata;
